...
export function CalendarScreen({ state }: { state: AppSnapshot }) {
  ...
  const wide = week || month;

  return (
    <div className="calendar-page" data-testid="calendar-page">
      <CalendarChrome state={state} />
      <div className={`calendar-day-layout ${wide ? "calendar-week-layout" : ""}`}>
        ...
        {month ? <MonthBoard state={state} /> : null}
        {wide ? null : <SummarySidebar state={state} onEditTarget={() => setPrompt({ kind: "target" })} />}
      </div>
